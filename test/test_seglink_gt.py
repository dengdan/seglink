...
feat_shapes = config.feat_shapes
...
def _test_cal_seglink_gt():
    ...
    labels, seg_gt, link_gt = get_all_seglink_gt(xs, ys, bbox_idx_in_label = True)
    inter_layer_link_gt, cross_layer_link_gt = reshape_link_gt_by_layer(link_gt)
    
    # test from here
    image = util.img.black((image_height, image_width, 3))
...
#                                 draw_horizontal_rect(img, nanchor, color = util.img.COLOR_GREEN)
                ...
                print "check the image, tell if it is right: No. green anchors == num_linkage", util.sit(img)
...
def _test_combine_seglinks():
    ...
    xs, ys = points_to_xys(points)
    anchors, layer_anchors = anchor_layer.generate_anchors()
    labels, seg_gt, link_gt = get_all_seglink_gt(xs, ys)

    seg_scores = labels
    seg_groups = group_segs(seg_scores = seg_scores, link_scores = link_gt)
    ...
    bboxes = seglink_to_bbox(seg_scores, link_scores = link_gt, segs = seg_gt, return_bias = draw_center_line)
    ...
    print "check the output image, make sure that all white bboxes are overridden by green ones.", util.sit(img)
